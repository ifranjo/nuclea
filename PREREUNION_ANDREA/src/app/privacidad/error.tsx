...
export default function PrivacidadError({
  error,
  reset,
}: {
  ...
  useEffect(() => {
    console.error('[privacidad-error]', error)
  }, [error])
  ...
  return (
    <main className="min-h-screen bg-[#0D0D12] flex items-center justify-center px-6">
      <div className="max-w-md w-full rounded-2xl border border-white/10 bg-white/5 p-6 backdrop-blur-sm">
        <h1 className="text-xl font-medium text-white mb-3">Error en privacidad</h1>
        <p className="text-sm text-white/60 mb-6">
          No se pudo cargar la política de privacidad. Puedes reintentar.
        </p>
